class StrExtension:
    ...
    def remove_vowels(str):
        return ''.join([s for s in str if s not in 'aeiouyAEIOUY'])
    @staticmethod
    def leave_alpha(str):
        return ''.join([s for s in str if s in 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'])

    @staticmethod
    def replace_all(str1, chars, char):
        res = str1
        for s1 in str1:
            if s1 in chars:
                res = res.replace(s1, char)
        return  res
